...
class PredictionConvolutions(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        self.loc_conv4_3_vis = nn.Conv2d(512, n_boxes['conv4_3'] * 4, kernel_size=3, padding=1)
        self.loc_conv6_vis = nn.Conv2d(512, n_boxes['conv6'] * 4, kernel_size=3, padding=1)
        self.loc_conv7_vis = nn.Conv2d(512, n_boxes['conv6'] * 4, kernel_size=3, padding=1)
        self.loc_conv8_vis = nn.Conv2d(512, n_boxes['conv7'] * 4, kernel_size=3, padding=1)
        self.loc_conv9_vis = nn.Conv2d(512, n_boxes['conv8'] * 4, kernel_size=3, padding=1)
        self.loc_conv10_vis = nn.Conv2d(512, n_boxes['conv9'] * 4, kernel_size=3, padding=1)

        self.loc_conv4_3_lwir = nn.Conv2d(512, n_boxes['conv4_3'] * 4, kernel_size=3, padding=1)
        self.loc_conv6_lwir = nn.Conv2d(512, n_boxes['conv6'] * 4, kernel_size=3, padding=1)
        self.loc_conv7_lwir = nn.Conv2d(512, n_boxes['conv6'] * 4, kernel_size=3, padding=1)
        self.loc_conv8_lwir = nn.Conv2d(512, n_boxes['conv7'] * 4, kernel_size=3, padding=1)
        self.loc_conv9_lwir = nn.Conv2d(512, n_boxes['conv8'] * 4, kernel_size=3, padding=1)
        self.loc_conv10_lwir = nn.Conv2d(512, n_boxes['conv9'] * 4, kernel_size=3, padding=1)
    ...
    def forward(self, conv4_3_feats, conv6_feats, conv7_feats, conv8_feats, conv9_feats, conv10_feats):
        ...
        l_conv4_3_vis = self.loc_conv4_3_vis(conv4_3_feats)
        l_conv4_3_vis = l_conv4_3_vis.permute(0, 2, 3, 1).contiguous()
        # (.contiguous() ensures it is stored in a contiguous chunk of memory, needed for .view() below)
        l_conv4_3_vis = l_conv4_3_vis.view(batch_size, -1, 4)

        l_conv6_vis = self.loc_conv6_vis(conv6_feats)
        l_conv6_vis = l_conv6_vis.permute(0, 2, 3, 1).contiguous()
        l_conv6_vis = l_conv6_vis.view(batch_size, -1, 4)

        l_conv7_vis = self.loc_conv7_vis(conv7_feats)
        l_conv7_vis = l_conv7_vis.permute(0, 2, 3, 1).contiguous()
        l_conv7_vis = l_conv7_vis.view(batch_size, -1, 4)

        l_conv8_vis = self.loc_conv8_vis(conv8_feats)
        l_conv8_vis = l_conv8_vis.permute(0, 2, 3, 1).contiguous()
        l_conv8_vis = l_conv8_vis.view(batch_size, -1, 4)

        l_conv9_vis = self.loc_conv9_vis(conv9_feats)
        l_conv9_vis = l_conv9_vis.permute(0, 2, 3, 1).contiguous()
        l_conv9_vis = l_conv9_vis.view(batch_size, -1, 4)

        l_conv10_vis = self.loc_conv10_vis(conv10_feats)
        l_conv10_vis = l_conv10_vis.permute(0, 2, 3, 1).contiguous()
        l_conv10_vis = l_conv10_vis.view(batch_size, -1, 4)

        l_conv4_3_lwir = self.loc_conv4_3_lwir(conv4_3_feats)
        l_conv4_3_lwir = l_conv4_3_lwir.permute(0, 2, 3, 1).contiguous()
        # (.contiguous() ensures it is stored in a contiguous chunk of memory, needed for .view() below)
        l_conv4_3_lwir = l_conv4_3_lwir.view(batch_size, -1, 4)

        l_conv6_lwir = self.loc_conv6_lwir(conv6_feats)
        l_conv6_lwir = l_conv6_lwir.permute(0, 2, 3, 1).contiguous()
        l_conv6_lwir = l_conv6_lwir.view(batch_size, -1, 4)

        l_conv7_lwir = self.loc_conv7_lwir(conv7_feats)
        l_conv7_lwir = l_conv7_lwir.permute(0, 2, 3, 1).contiguous()
        l_conv7_lwir = l_conv7_lwir.view(batch_size, -1, 4)

        l_conv8_lwir = self.loc_conv8_lwir(conv8_feats)
        l_conv8_lwir = l_conv8_lwir.permute(0, 2, 3, 1).contiguous()
        l_conv8_lwir = l_conv8_lwir.view(batch_size, -1, 4)

        l_conv9_lwir = self.loc_conv9_lwir(conv9_feats)
        l_conv9_lwir = l_conv9_lwir.permute(0, 2, 3, 1).contiguous()
        l_conv9_lwir = l_conv9_lwir.view(batch_size, -1, 4)

        l_conv10_lwir = self.loc_conv10_lwir(conv10_feats)
        l_conv10_lwir = l_conv10_lwir.permute(0, 2, 3, 1).contiguous()
        l_conv10_lwir = l_conv10_lwir.view(batch_size, -1, 4)
        ...
        locs_vis = torch.cat([l_conv4_3_vis, l_conv6_vis, l_conv7_vis, l_conv8_vis, l_conv9_vis, l_conv10_vis], dim=1)
        locs_lwir = torch.cat([l_conv4_3_lwir, l_conv6_lwir, l_conv7_lwir, l_conv8_lwir, l_conv9_lwir, l_conv10_lwir], dim=1)
        classes_scores = torch.cat([c_conv4_3, c_conv6, c_conv7, c_conv8, c_conv9, c_conv10],
                                   dim=1)

        return locs_vis, locs_lwir, classes_scores
...
class SSD300(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        if args['train'].load_model != "":
            self.load_pretrained_dual_regressor_model()

        if args['train'].tune_regressor_only:
            self.tune_regressor_only()

    def load_pretrained_dual_regressor_model(self):
        ## load pretrained model ##

        state_dict = self.state_dict()
        param_names = list(state_dict.keys())

        pretrained_state_dict = torch.load(args['train'].load_model)['model'].state_dict()
        ...
        print("\nInitialize weights from previous trained model.\n")

    def tune_regressor_only(self):

        for param_name, param in self.named_parameters():
            if 'loc_conv' not in param_name:
    ...
    def forward(self, image_vis, image_lwir):
        ...
        locs_vis, locs_lwir, classes_scores = self.pred_convs(conv4_3_feats, conv6_feats, conv7_feats, conv8_feats, conv9_feats, conv10_feats)  # (N, 8732, 4), (N, 8732, n_classes)
        return locs_vis, locs_lwir, classes_scores
    ...
    def detect_objects(self, predicted_locs_vis, predicted_locs_lwir, predicted_scores, min_score, max_overlap, top_k):
        """
        ...
        all_images_boxes_vis = list()
        all_images_boxes_lwir = list()
        all_images_labels = list()
        all_images_scores = list()
        all_images_bg_scores = list()

        assert n_priors == predicted_locs_vis.size(1) == predicted_locs_lwir.size(1) == predicted_scores.size(1)

        for i in range(batch_size):
            # Decode object coordinates from the form we regressed predicted boxes to
            ### decoded_locs_vis and decoded_locs_lwir have the same length with different values on indice of paired objects. ###
            decoded_locs_vis = cxcy_to_xy(gcxgcy_to_cxcy(predicted_locs_vis[i], self.priors_cxcy))
            decoded_locs_lwir = cxcy_to_xy(gcxgcy_to_cxcy(predicted_locs_lwir[i], self.priors_cxcy))

            # Lists to store boxes and scores for this image
            image_boxes_vis = list()
            image_boxes_lwir = list()
            image_labels = list()
            image_scores = list()
            ...
            class_decoded_locs_vis = decoded_locs_vis[score_above_min_score]
            class_decoded_locs_lwir = decoded_locs_lwir[score_above_min_score]

            # Sort predicted boxes and scores by scores\
            ### Sort scores by average between modals ###
            ...
            class_decoded_locs_vis = class_decoded_locs_vis[sort_ind]  # (n_min_score, 4)
            class_decoded_locs_lwir = class_decoded_locs_lwir[sort_ind]  # (n_min_score, 4)

            # Find the overlap between predicted boxes

            overlap_mmiou, overlap_visiou, overlap_lwiriou = find_mmiou_overlap_classify_all_iou(class_decoded_locs_vis, class_decoded_locs_lwir,
                                                  class_decoded_locs_vis,
                                                  class_decoded_locs_lwir, class_scores, class_threshold_1=0.1,
                                                  class_threshold_2=0.1)  # (n_qualified, n_min_score)

            # Non-Maximum Suppression (NMS)
            suppress = torch.zeros((n_above_min_score), dtype=torch.bool).to(device)  # (n_qualified)

            # Consider each box in order of decreasing scores
            for box in range(class_decoded_locs_vis.size(0)):
                # If this box is already marked for suppression
                if suppress[box] == 1:
                    continue

                suppress = torch.max(suppress, overlap_mmiou[box] > max_overlap)
                suppress = torch.max(suppress, overlap_visiou[box] > 0.75)
                suppress = torch.max(suppress, overlap_lwiriou[box] > 0.75)
                suppress[box] = 0

            # Store only unsuppressed boxes for this class
            image_boxes_vis.append(class_decoded_locs_vis[~suppress])
            image_boxes_lwir.append(class_decoded_locs_lwir[~suppress])
            image_labels.append(torch.ones((~suppress).sum().item()).to(device))
            image_scores.append(class_scores[~suppress])
            image_bf_scores.append(bg_scores[~suppress])

            # If no object in any class is found, store a placeholder for 'background'
            # assert image_boxes_vis.size(0) == image_boxes_lwir.size(0)
            if len(image_boxes_vis) == 0 or len(image_boxes_lwir) == 0:
                image_boxes_vis.append(torch.FloatTensor([[0., 0., 1., 1.]]).to(device))
                image_boxes_lwir.append(torch.FloatTensor([[0., 0., 1., 1.]]).to(device))
                image_labels.append(torch.LongTensor([0]).to(device))
                image_scores.append(torch.FloatTensor([0.]).to(device))
                image_bf_scores.append(torch.FloatTensor([0.]).to(device))

            # Concatenate into single tensors
            image_boxes_vis = torch.cat(image_boxes_vis, dim=0)
            image_boxes_lwir = torch.cat(image_boxes_lwir, dim=0)
            ...
            if n_objects > top_k:
                ...
                image_boxes_vis = image_boxes_vis[sort_ind][:top_k]  # (top_k, 4)
                image_boxes_lwir = image_boxes_lwir[sort_ind][:top_k]  # (top_k, 4)
                image_labels = image_labels[sort_ind][:top_k]  # (top_k)
                image_bg_scores = image_bg_scores[sort_ind][:top_k]

            # Append to lists that store predicted boxes and scores for all images
            all_images_boxes_vis.append(image_boxes_vis)
            all_images_boxes_lwir.append(image_boxes_lwir)
            all_images_labels.append(image_labels)
            all_images_scores.append(image_scores)
            all_images_bg_scores.append(image_bg_scores)

        return all_images_boxes_vis, all_images_boxes_lwir, all_images_labels, all_images_scores, all_images_bg_scores  # lists of length batch_size
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs_vis, predicted_locs_lwir, predicted_scores, boxes_vis, boxes_lwir, labels):
        ...
        true_locs_vis = torch.zeros((batch_size, n_priors, 4), dtype=torch.float, device=device)
        true_locs_lwir = torch.zeros((batch_size, n_priors, 4), dtype=torch.float, device=device)
        ...
        for i in range(batch_size):
            ...
            overlap = find_mmiou_overlap_expand_BB_label(boxes_vis[i], boxes_lwir[i], self.priors_xy, self.priors_xy, labels[i])
            ...
            true_locs_vis[i] = cxcy_to_gcxgcy(xy_to_cxcy(boxes_vis[i][object_for_each_prior]), self.priors_cxcy)  # (8732, 4)
            true_locs_lwir[i] = cxcy_to_gcxgcy(xy_to_cxcy(boxes_lwir[i][object_for_each_prior]), self.priors_cxcy)  # (8732, 4)

        # Identify priors that are positive (object/non-background)
        positive_priors = true_classes > 0  # (N, 8732)
        positive_priors_vis = torch.logical_or(true_classes == 1, true_classes == 3 )
        positive_priors_lwir = torch.logical_or(true_classes == 2, true_classes == 3 )

        # LOCALIZATION LOSS #
        # Localization loss is computed only over positive (non-background) priors
        assert true_locs_vis[positive_priors].shape[0] == true_locs_lwir[positive_priors].shape[0]
        if true_locs_vis[positive_priors_vis].shape[0] == 0 and true_locs_lwir[positive_priors_lwir].shape[0] == 0:
            no_positive_prior = torch.tensor(0., device=device)
            no_positive_prior.requires_grad = True
            loc_loss = no_positive_prior
        elif true_locs_vis[positive_priors_vis].shape[0] != 0 and true_locs_lwir[positive_priors_lwir].shape[0] == 0:
            loc_loss = self.smooth_l1(predicted_locs_vis[positive_priors_vis], true_locs_vis[positive_priors_vis]) # (), scalar
        elif true_locs_vis[positive_priors_vis].shape[0] == 0 and true_locs_lwir[positive_priors_lwir].shape[0] != 0:
            loc_loss = self.smooth_l1(predicted_locs_lwir[positive_priors_lwir], true_locs_lwir[positive_priors_lwir]) # (), scalar
        else:
            loc_loss = self.smooth_l1(predicted_locs_vis[positive_priors_vis], true_locs_vis[positive_priors_vis]) + \
                       self.smooth_l1(predicted_locs_lwir[positive_priors_lwir], true_locs_lwir[positive_priors_lwir]) # (), scalar
