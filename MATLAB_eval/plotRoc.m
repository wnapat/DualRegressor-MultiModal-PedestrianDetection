...
set(get(get(h,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');% 使这次plot不在legend里出现
...
if( m>0 )
  ...
  fp=min(fpTarget); 
  h=plot([fp fp],lims(3:4),'Color',.7*[1 1 1]);  % 给个返回值h
  set(get(get(h,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');% 使这次plot不在legend里出现
  fp=max(fpTarget); 
  h=plot([fp fp],lims(3:4),'Color',.7*[1 1 1]);  % 给个返回值h
  set(get(get(h,'Annotation'),'LegendInformation'),'IconDisplayStyle','off') % 使这次plot不在legend里出现
...
if( logy==1 )
  ticks=[.001 .002 .005 .01 .02 .10 .20 .30 .40 .50 .64 .80 1];   % 在y轴上多显示几个刻度
  set(gca,'YScale','log','YTick',ticks);
end
if( logx==1 || logy==1 ), grid on;
  set(gca,'XMinorGrid','off','XMinorTic','off');
  set(gca,'YMinorGrid','on','YMinorTic','on');   % 开启 用虚线打更密的网格
end
