...
function [objs,bbs] = bbLoad( fName, varargin, shift )
% Load bb annotation from text file and filter.
%
...
if( format==0 )
  ...
  bb=[in{2}+shift in{3} in{4} in{5}]; bbv=[in{7} in{8} in{9} in{10}];
...
function [fs,fs0] = getSubsetFiles( dirs, subset, inputType, f0, f1 )
% Get all corresponding files in given directories.
%
% The first dir in 'dirs' serves as the baseline dir. getFiles() returns
% all files in the baseline dir and all corresponding files in the
...
if(nargin<3 || isempty(inputType)), inputType=''; end
...
info=textscan(fp, '%s%s%s', 'Delimiter', '/');  fclose(fp);

[fs1,fs0] = getFiles0(dirs{1},info,inputType,f0,f1);
...
% for d=2:m, fs(d,:)=getFiles1(dirs{d},fs0,sep); end
  ...
  function [fs1,fs0] = getFiles0( dir1, info, inputType, f0, f1 )
    ...
    if isempty(inputType)
        fs0=info{1}(f0:min(f1,end));
        fs1=cellfun(@(x) fullfile(dir1, [x ext]), ...
            info{1}(f0:min(f1,end)), 'uniformoutput', false );  
    elseif contains(dir1,'cvc-14')
        fs0=info{1}(f0:min(f1,end));
        fs1=cellfun(@(x) fullfile(dir1, inputType, [x ext]), ...
            info{1}(f0:min(f1,end)), 'uniformoutput', false );  
    else
...
function [gt0,dt0] = loadAll( gtDir, dtDir, pLoad, inputType, subset, shift )
% Load all ground truth and detection bbs in given directories.
%
...
if(isempty(dtDir)), fs=getSubsetFiles( {gtDir}, subset, inputType ); gtFs=fs(1,:); else
  dtFile=length(dtDir)>4 && strcmp(dtDir(end-3:end),'.txt');
  if(dtFile), dirs={gtDir}; else dirs={gtDir,dtDir}; end
  fs=getSubsetFiles(dirs, subset, inputType); gtFs=fs(1,:);
  if(dtFile), dtFs=dtDir; else dtFs=fs(2,:); end
end

key={gtDir,pLoad}; n=length(gtFs);
gt0=cell(1,n);
% gtFs is a (1,n) cell, contains file name(str)
for i=1:n, [~,gt0{i}]=bbLoad(gtFs{i},pLoad,shift);
    if size(gt0{i},1) ~= 0
        for j=1:size(gt0{i},1)
        end
    end
end   
...
% get parameters
if(nargin<4 || isempty(thr)), thr=.5; end
if(nargin<5 || isempty(mul)), mul=0; end
...
% Attempt to ma                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             tch each (sorted) dt to each (sorted) gt
...
%  ref        - [] reference points for ROC or PR curve  %(FPPI)
%
% OUTPUTS
